...
重启后，新笔记仍在列表中，标题与正文保持不变。首次运行会创建 `<userData>/notes/notes.json`。页面状态显示“本地笔记仓库已连接”，footer 与编辑器分别显示“保存在本地”和“已保存到本地”。
...
| 页面显示本地仓库已连接，但目录中没有 JSON | 启动加载尚未完成，或初始化写入失败 | 查看状态区错误，检查 `userData` 路径与目录写权限 |
